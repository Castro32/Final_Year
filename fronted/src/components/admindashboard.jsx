...
class AdminDashboard extends Component {
  ...
  handleEditSubmit = async () => {
    ...
    try {
      ...
      const [name, ...lastNameParts] = currentEditItem.name.split(' ');
      ...
      if (editType === 'doctor') {
        ...
        body = {
          ...
          medications: currentEditItem.medications,
          ...(currentEditItem.password && { password: currentEditItem.password })
        };
      }
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, pt: 1 }}>
              ...
              {editType === 'patient' && (
                <>
                  ...
                  <TextField
                    label="Medications"
                    value={currentEditItem.medications || ''}
                    onChange={(e) => this.handleEditFieldChange('medications', e.target.value)}
                    fullWidth
                    margin="dense"
                    multiline
                    rows={2}
